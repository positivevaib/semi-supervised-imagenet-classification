...
parser.add_argument('--model',
                    type=str,
                    default=None,
                    help='absolute path to save or load model')
...
if args.resume:
    model = torch.load(args.checkpoint['model_state_dict'],
                       map_location=device)
    optimizer = torch.load(args.checkpoint['optimizer_state_dict'],
                           map_location=device)
    scheduler = torch.load(args.checkpoint['scheduler_state_dict'],
                           map_location=device)
...
if not args.predict:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        tqdm.tqdm.write('epoch {} of {}'.format(epoch + 1, args.epochs),
                        file=args.file)
        ...
        for batch_idx, data in enumerate(train_loader):
                ...
                if not plot:
                    ...
                    viz.line(X=np.array([
                        ...
                        ((epoch * len(train_loader)) + batch_idx + 1)
                             ...
                             Y=np.array([loss.item(), loss.item()]),
                             env=env_name,
                             win=plot,
                             name='train',
                             update='append')
            ...
            if batch_idx == 0:
                tqdm.tqdm.write('initial training loss: {:.2f}'.format(
                    loss.item()),
                                file=args.file)
        ...
        tqdm.tqdm.write(
            'training loss: {:.2f}, validation loss: {:.2f}\n'.format(
                running_loss / (batch_idx + 1), avg_val_loss),
            file=args.file)
...
# evaluate model
    ...
    test_loader = data.get_data_loaders(args.data,
                                        test_loader=True,
