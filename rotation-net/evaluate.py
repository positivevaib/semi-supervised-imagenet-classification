...
def evaluate(model, dataloader, data_size, criterion, device, pbar, pbar_file):
    ...
    # setup progress bar, if applicable
    if pbar:
        desc = "ITERATION - loss: {:.2f}"
        pbar_ = tqdm.tqdm(desc = desc.format(0), total = len(dataloader), leave = False, file = pbar_file, initial = 0)
    ...
    with torch.no_grad():
        ...
        # close progress bar, if applicable, and print stats
        if pbar:
            pbar_.close()
            tqdm.tqdm.write('val loss: {:.2f}, val accuracy {:.2f}%\n'.format(running_loss / (batch_idx + 1), 
                                                                                (correct * 100) / data_size), file = pbar_file)
